...
import {createJurusan,deleteJurusan,fetchJurusan,updateJurusan,} from "../../services/jurusanService.js";
...
export default function JurusanPage() {
  ...
  async function loadData() {
    try {
      ...
      const [jurusanData, fakultasData] = await Promise.all([
        fetchJurusan(),
        fetchFakultas(),
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        fakultas_id: Number(formData.fakultas_id),
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setJurusan((prev) => prev.filter((item) => item.id !== id));
          ...
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Kelola data jurusan berdasarkan fakultas.
              </p>
            </div>
            ...
            <div className="w-full max-w-2xl overflow-hidden rounded-[28px] bg-white shadow-2xl ring-1 ring-slate-200">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <label className="block text-sm text-slate-700">
                  <span className="mb-2 block">Fakultas</span>

                  <select
                    value={formData.fakultas_id}
                    onChange={(e) =>
                      handleChange("fakultas_id", e.target.value)
                    }
                ...
                <label className="block text-sm text-slate-700">
                  <span className="mb-2 block">Kode Jurusan</span>

                  <input
                    type="text"
                    value={formData.kode_jurusan}
                    onChange={(e) =>
                      handleChange("kode_jurusan", e.target.value)
                    }
                ...
                <label className="block text-sm text-slate-700">
                  <span className="mb-2 block">Nama Jurusan</span>

                  <input
                    type="text"
                    value={formData.nama_jurusan}
                    onChange={(e) =>
                      handleChange("nama_jurusan", e.target.value)
                    }
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    {editingId ? "Update Jurusan" : "Simpan Jurusan"}
                  </button>
